...
router.get('/', (req, res) => {
  db('cars')
    ...
    .catch(err => {
      ...
      res
        .status(500)
        .json({ error: 'Could not get cars information from database' });
    });
});
  ...
  db('cars')
    ...
    .then(count => {
      if (count) {
        res
          .status(200)
          .json({ message: 'Successfully updated Car information' });
      } else {
  ...
  db('cars')
    ...
    .then(count => {
      if (count) {
        res
          .status(200)
          .json({ message: 'Successfully deleted the car deleted' });
      } else {
    ...
    .catch(err => {
      res
        .status(500)
        .json({ message: "Could not delete car, can't access database" });
    });
});
...
function validateCar(req, res, next) {
  ...
  if (VIN.length !== 17) {
    return res
      .status(400)
      .json({ error: 'VIN is not possible, VIN should have 17 characters.' });
  }
  next();
}
